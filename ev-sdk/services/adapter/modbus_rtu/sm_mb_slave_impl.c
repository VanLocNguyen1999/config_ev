...
int32_t sm_mb_slave_init(sm_mb_slave_t* _this){
    ...
    _impl(_this)->m_nmbs_if.arg = _impl(_this)->m_arg;
    ...
    nmbs_set_byte_timeout(&_impl(_this)->m_nmbs_instance, 100);
    nmbs_set_read_timeout(&_impl(_this)->m_nmbs_instance, 100);
...
int32_t nmbs_read_fn(uint8_t* buf, uint16_t count, int32_t byte_timeout_ms, void* arg){
    return g_mb_slave.m_recv_fn_t(_impl(arg)->m_addr, buf, count, byte_timeout_ms, arg);
}

int32_t nmbs_write_fn(const uint8_t* buf, uint16_t count, int32_t byte_timeout_ms, void* arg){
    return g_mb_slave.m_send_fn_t(_impl(arg)->m_addr, buf, count, byte_timeout_ms, arg);
}
