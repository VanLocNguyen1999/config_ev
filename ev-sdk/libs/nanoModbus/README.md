...
int main(int argc, char* argv[]) {
    ...
    platform_conf.m_cb_arg = conn;    // Passing our TCP connection handle to the read/write functions
    ...
    // Set only the response m_timeout. Byte m_timeout will be handled by the TCP connection
...
```C
int32_t read(uint8_t* buf, uint16_t m_count, int32_t byte_timeout_ms, void* m_cb_arg);
int32_t write(const uint8_t* buf, uint16_t m_count, int32_t byte_timeout_ms, void* m_cb_arg);
```
...
Platform functions and server callbacks can access arbitrary user data through their `void* m_cb_arg` argument. The argument
